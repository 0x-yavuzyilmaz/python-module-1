...
eylem = input("Ne yapmak istersin? Hangi eşya ile başlayalım? Kapı, sandık, pencere?").lower()
...
if eylem == "kapı":
    ...
    print(f"{eylem} ile ne yapacağımı bilmiyorum.Lütfen 'kapı', 'sandık' veya 'pencere' gibi nesnelerle etkileşime geç. ")

anahtar_alindi_mi=input("Evet doğru yoldasın, sandığı açman için gerekiyor").lower()
if anahtar_alindi_mi == "anahtar":
    print("Anahtarı kullanarak sandığı açtın ve içinden bir levye çıktı!")
else:
    print("Sandık hala kilitli:(")
